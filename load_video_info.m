function [img_files, pos, target_sz, ground_truth, video_path, init_gt] = load_video_info(base_path, video)
%LOAD_VIDEO_INFO
...
% 	filename = [video_path 'groundtruth_rect0', num2str(i), suffix '.txt'];
    ...
    init_gt = ground_truth;
	fclose(f);
	
	%set initial position and size
	target_sz = [ground_truth(1,4), ground_truth(1,3)];
	pos = [ground_truth(1,2), ground_truth(1,1)] + (target_sz/2);
	...
	if size(ground_truth,1) == 1,
		...
		ground_truth = [ ground_truth(:,[2,1]) + ground_truth(:,[4,3]) / 2, ground_truth(:,[4,3])];
	end
...
%             img_files = dir([video_path '*.png']);
		...
		if exist(sprintf('%s.0%04i.png', video_path, frames{idx,2}), 'file'),
